(() => {
  ...
  class Pd03ProductFinder {
    ...
    resize() {
      ...
      this.timer = setTimeout(() => {
        ...
        if (BREAKPOINTS.MOBILE < utils.getViewPort().width) {
          ...
          this.el.component.find('.pd03-product-finder__compare-bar-box').attr('aria-live', 'polite');
          this.el.component.find('.pd03-product-finder__compare-bar-cta > .cta--compare').removeAttr('aria-live'); 
          ...
          this.el.component.find('.pd03-product-finder__compare-bar-box').removeAttr('aria-live');
          this.el.component.find('.pd03-product-finder__compare-bar-cta > .cta--compare').attr('aria-live', 'polite');
        }
    ...
    stickyHeader() {
      const top = document.body.scrollTop || document.documentElement.scrollTop;
      const exploreH = $q('.explore-lnb.sticky-nav').height();
      const visualH = $q('.nv14-visual-lnb__container.sticky').height();
      const exploreHNV07 = $q('.nv07-explore-floating-navigation.use-sticky-nav').height();

      let fixedH;
      if (this.isExplorerLnb) {
        fixedH = this.el.filterBar.height() + exploreH;
      } else if (this.isVisualLnb) {
        fixedH = this.el.filterBar.height() + visualH;
      } else if (this.isExplorerLnbNV07) {
        fixedH = this.el.filterBar.height() + exploreHNV07;
      } else {
  ...
  const setCompareBarItem = () => {
    ...
    Object.keys(infoObj).forEach((key, idx) => {
      ...
      item.find('.pd03-product-finder__compare-bar-item-product-name').text(obj.title);

      if(obj.description){
        item.find('.pd03-product-finder__compare-bar-item-description').show();
        item.find('.pd03-product-finder__compare-bar-item-description').text(obj.description);
      }
...
(() => {
	...
	const LOWEST_WAS_PRICE_STRIKETHROUGH_SITE_CODE = ["pl", "gr", "si", "fi"]; // lowestWasPrice strikethrough siteCode 
	...
	var buildProductInfoArea = function(product){
			...
			if(usePriceArea) {
				...
				if(isGPv2 || isNewHybris) {
					...
					if(siteCode === "it"){
						...
						if(isNotNull(frontModel.lowestWasPrice) && isNotNull(frontModel.promotionPrice) 
							...
							let savePriceText = Granite.I18n.get("Save {0}", [currencyComma(savePrice, pfPriceCurrency)]);
							topPrice = `
							<span class="quick-view__price-origin">
								<span class="hidden">${Granite.I18n.get("Original Price")}: </span>
								${savePriceText} (<del>${frontModel.lowestWasPriceDisplay}</del>)
							</span>
							`;
						}
					}
				...
				if((upperStock === "LEARNMORE" || upperStock === "NOORDER") && (siteCode === "nl" || siteCode === "be" || siteCode === "be_fr")) {
					...
					if(isNotNull(tradeInPriceText)) {
						...
						if(leasingInterest !== "" && siteCode !== "pe" && siteCode !== "cl" && siteCode !== "mx" && siteCode !== "co"){
							priceTextForCompare +=
						...
						if(siteCode === "br") {
							...
							if(siteCode === "pe" || siteCode === "cl" || siteCode === "mx" || siteCode === "co" || siteCode === "my"){
								mainPrice = `
									<span class="hidden">${Granite.I18n.get("Total Price")}: </span>
									${displayText}
								`;
				...
				if(pkgChildModels.length > 0){
					...
					for(var pkgChildModelIdx in pkgChildModels){
						...
						if(isNotNull(pkgChildModel.repairabilityIndex)){
							let repairabilityIndex = pkgChildModel.repairabilityIndex;
		...
		var buildFicheArea = function(frontModel, thirdPASeller) {
			...
			if(priceExposed) {
				// productCardTemplate = labelText !== '' ? `
				// <div class="quick-view__fiche">
				// 	${labelText}
				// </div>
				// ` : "";
				...
				productCardTemplate = `
				<div class="quick-view__fiche no-data"></div>
				`;
			}
			return productCardTemplate;
		}
	...
	var quickviewEventListener = function(product){
		...
		$ctaBtn.on("click.finder", function(e){
			...
			if(isGPv2){
				...
				let cartActionId = COMMON_NETFUNNEL_ACTIONID.AEM_ADD != undefined && COMMON_NETFUNNEL_ACTIONID.AEM_ADD != "" ? COMMON_NETFUNNEL_ACTIONID.AEM_ADD : "AddtoCart_aem";
...
(function (win, $) {
	...
	if(getBrowserName() !== "Safari" && !isOldAndroid){
...
//	};
	...
	var productFinderGrid = function(){
		...
		const OLD_HYBRIS_WISHLIST_OPEN = ["hk", "hk_en"]; // old hybris Wishlist open
		...
		var buildProductCardV2 = function(product) {
			...
			var productCardPriceAreaBuild = function(product, upperStock, thirdPASeller, cardLayout) {
				...
				if(usePriceArea) {
						...
						if(isNotNull(tradeInPriceText)) {
							...
							if(siteCode === "br") {
								...
								if(siteCode === "pe" || siteCode === "cl" || siteCode === "mx" || siteCode === "co" || siteCode === "my"){
									...
									mainPrice = `
									<span class="hidden">${Granite.I18n.get("Total Price")}: </span>
									<span class="pd03-product-card__price-total-unit">${currentDisplay}</span>
									`;
								}else{
									mainPrice = `
									<span class="hidden">${Granite.I18n.get("Total Price")}: </span>
					...
					if ( isEppSite && siteCode === "pl" ){
                    	...
                    	if ( isNotNull(frontModel.msrpPrice) ){
                    		msrpPrice = ' ' + currencyComma(frontModel.msrpPrice, pfPriceCurrency);
							advicedPriceDiv = `<div class="pd03-product-card__finance">${Granite.I18n.get("Listprice")}${msrpPrice}</div>`;
                    	} else {
					...
					// mainPriceAttr
					if(siteCode === "ro" && frontModel.promotionPrice !== frontModel.price){
						priceMainClass = `pd03-product-card__price-total`;
					}
				...
				const seromClass = (siteCode === "ro" && frontModel.promotionPrice !== frontModel.price) ? "pd03-product-card__price-serom" : "";
			...
			var productCardEnergyLabelBuild = function(frontModel, cardLayout) {
				...
				const ficheInner = priceExposed? `${labelText.trim()}${repairabilityHtml.trim()}`.trim() : "";
